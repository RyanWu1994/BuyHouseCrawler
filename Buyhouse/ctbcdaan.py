...
url = "https://buy.cthouse.com.tw/area/1-50-year/臺北市-city/大安區-town/0-5000-price/page1.html"
headers = {"User-Agent":ua.random}
...
for j in range (1,lastPage):
    ...
    ua = UserAgent()
    url = "https://buy.cthouse.com.tw/area/1-50-year/臺北市-city/大安區-town/0-5000-price/page"+str(j)+".html"
    headers = {"User-Agent":ua.random}
    res = requests.get(url,headers=headers)
    soup = BeautifulSoup(res.text)
    sel = etree.HTML(res.content)
...
for n in range(0,len(Name)):
    Daan.append({"Name":Name[n],"Address":Address[n],"Price":Price[n],"Year":Year[n],"Area":Area[n],"Pattern":Pattern[n],"Store":"中信"})
